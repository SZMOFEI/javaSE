...
public static void main(String[] args) {
    //1.在控制输出10次helloworld,这样做不推荐,因为复用性太差
    /*
    ...
    */
    ...
    //2.用循环语句来解决:先初始化,然后判断成立,走语句体,然后走自增,如果判断不成立就结束循环
    for (int i = 1;i <= 10 ;i++ ) {
    	System.out.println("helloworld");
    }
...
public static void main(String[] args) {
    for (int i = 1;i <= 10 ;i++ ){
    	System.out.println("i = " + i);//控制台输出1-10
    }
    
    System.out.println("----------分割线-------------");
    
    for (int i = 10;i >= 1 ;i-- ) {
    	System.out.println("i = " + i);//控制台输出10-1
    }
...
```
public static void main(String[] args) {
    for (int i = 100;i <= 999 ;i++ ){//获取100到999之间的数即三位数
    
        int ge =  i / 1 % 10;    //123 % 10 
    	int shi = i / 10 % 10;  //12 % 10
    	int bai = i / 100 % 10; //1 % 10
        //发现规律没?假如要你求十万位的数字是多少?你懂了吗?
...
public static void main(String[] args) {
    ...
    */
    ...
    //另外,某屌丝为了追求女神,写了一段代码多次示爱,但是女神也会java,改动一下就把屌丝拒绝了,你知道怎么做吗?代码如下:
    int j = 1;
    while (j <= 10000) {//{括号左边加个分号;即可
...
public static void main(String[] args) {
    ...
    */
    ...
    //System.out.println("i = " + i);//报错,for语句执行后变量会被释放,不能再使用
    System.out.println("-------------------");
    ...
    System.out.println("-------------------");
    System.out.println("i = " + i);	//11,while语句执行后,初始化变量还可以继续使用
    */
...
public static void main(String[] args) {
    ...
    //System.out.println("hello world");//无限循环下面不能带语句,否则编译报错:无法到达的代码
...
public static void main(String[] args) {
    ...
    for (int i = 1;i <= 4 ;i++ ) {	   //外循环决定的是行数
    	for (int j = 1;j <= 5 ;j++ ) { //内循环决定的是列数
    		System.out.print("*");//有一种一对多的关系,感受下!
    	}
    	System.out.println();//1对5后才换行
    }
...
public static void main(String[] args) {
    for (int i = 1;i <= 9 ;i++ ) {		//行数
    	for (int j = 1;j <= i ;j++ ) {	//列数
    		System.out.print(j + "*" + i + "=" + (i * j) + "\t" );//一对多的关系,感受下!
    	}
    	
    	System.out.println();//上面不换行,每i个输出换一次行
    }
...
public static void main(String[] args) {
    for (int x = 1;x <= 10 ;x++ ) {
    	...
    	System.out.println("x = " + x);//分别输出1,2,3
    }
...
public static void main(String[] args) {
    for (int x = 1;x <= 10 ;x++ ) {
    	if (x == 4) {
    		continue;//下面不会输出4,因为continue语句的作用是:终止本次循环,继续下一次循环
    	}
    
    	System.out.println("x = " + x);//除了4,1到10都有输出
    }
...
public static void main(String[] args) {
    ...
    outer: for (int i = 1;i <= 10 ;i++ ) {//outer就是标号,只要是合法的标识符即可
    	...
    	inner: for (int j = 1;j <= 10 ;j++ ) {
    		System.out.println("j = " + j);
    		break outer;//跳出标号对应的循环,这里是外循环,也可以是内循环如break inner;
    	}
    ...
    */
    ...
    http://www.heima.com //这个是标号加注释也不会报错,因为下面还有一个输出语句或者是空语句;
    System.out.println("才是真的好");//如果这里连空语句;也没有,上面标号加注释会报错,换言之↓
    //↑标号后面必须加语句或者代码块,这样都是没问题的http2:;	http2:{}
		
}
```

###04.17_Java语言基础(控制跳转语句练习)
A:练习题
...
public static void main(String[] args) {
    for (int i = 1;i <= 10 ;i++ ) {
    	if (i == 4) {				
    		//break;//停止循环
    		return;	//返回的意思,用来返回方法,这里是main方法!
    	}
    	
    }
    
    System.out.println("循环结束了");//return这行代码就不走了
}
```
...
class Demo3_Method {
	public static void main(String[] args) {
		...
		//错误: 此处不允许使用 '空' 类型,返回值是void的方法不能输出调用
...
###B:方法重载:
    在同一个类中(前提),方法名相同,参数列表不同,与返回值类型无关,(这样的多个方法就是方法的重载!)
	
	参数列表不同:(指的是满足下面任意一个条件即可)
	    A:参数个数不同
		B:参数类型不同
...
#04.27_day04总结
    ...
    break语句-continue语句-标号-return语句-
    方法定义与调用-
    方法重载
